...
import config
from db import (
    init_db, add_user, update_categories, get_user_categories,
...
# Используем сессию с числовым таймаутом
    ...
    if args.startswith("ref_"):
        try:
            ...
            if referrer_id != user_id and referrer_id > 10000:
                conn = sqlite3.connect("daily_digest.db")
                cursor = conn.cursor()
...
async def subscribe_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("Отметьте темы (галочка = выбрано):", reply_markup=categories_keyboard(current))
...
async def main():
    init_db()
    start_scheduler(bot)
    print("DailyDigest AI — запущен и работает 24/7")
